...
public class WroxAccessory {
	...
	public void connect(int mode, Connection connection, String ident)
			throws IOException {
		mMonitoringThread = new MonitoringThread(mode, connection);
		mMonitoringThread.mConnection.getOutputStream().write(
				MQTT.connect(ident));
	}
	...
	public String subscribe(BroadcastReceiver receiver, String topic, int id)
			throws IOException {
	...
	private class WriteHelper extends AsyncTask<byte[], Void, Void> {
		...
		protected Void doInBackground(byte[]... params) {
			try {
				...
				mMonitoringThread.mConnection.getOutputStream()
						.write(params[0]);
			} catch (IOException e) {
				e.printStackTrace();
			}
